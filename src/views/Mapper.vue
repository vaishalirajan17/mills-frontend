...
function create() {
  ...
  axios.post('http://localhost:8080/maprotor', request)
    .then(function (response) {
      ...
      startSuccess.value = "Mapping Created";
      buttonText.value = "CREATE MAPPING";
    })
    ...
    <div class="table-row-group bg-gray-800">
      <div class="table-row">
        <div class="table-cell text-left p-4 border-b-2 border-b-slate-700">
          <select class="bg-gray-700" name="mill" id="mill_id" v-model="selectedMillId">
        ...
        <div class="table-cell text-left p-4 border-b-2 border-b-slate-700">
          <select class="bg-gray-700" name="rotor" id="rotor_id" v-model="selectedRotorId">
        ...
        <div class="table-cell text-left p-4 border-b-2 border-b-slate-700">
          <div class="flex flex-row"><button class="py-2 px-8 rounded-md min-w-24 bg-lime-400 text-black"
              @click="create">{{ buttonText }}</button></div>
        </div>
      </div>
    </div>
  </div>
