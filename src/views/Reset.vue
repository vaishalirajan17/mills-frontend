 ...
 import axios from "axios";
  import {ref} from "vue";
    ...
    .then(function (response) {
      ...
      startSucess.value = "Reset sucessful";
      resetDisplay();
    })
...
<template>
  ...
  <table class="w-full text-sm text-left rtl:text-right text-gray-500 text-gray-400">
      ...
      <tbody>
          <tr v-for = "comp in comps" class="bg-slate-900 border-b bg-gray-800 border-gray-700 text-white">
              ...
              <td class="px-6 py-4">
                <input class="bg-slate-900 bg-gray-700 light:[color-scheme:light]" type="datetime-local" id="resetTime" v-model="resetTime">
              </td>
              <td class="px-6 py-4">
                {{comp.runningHours}}
              </td>
              <td class="px-6 py-4">
                <button class="bg-rose-700 bg-gray-700 py-2 rounded-md min-w-24 disabled:bg-gray-300" @click="reset(comp.millId, comp.rotorId, comp.componentName,comp.overAllDate)">RESET</button>
              </td>
          </tr>
      </tbody>
  </table>
  ...
  <div v-if="isStartSuccess" class="bg-green-100 border border-green-400 text-green-700  py-3 px-10 rounded relative" role="alert">
